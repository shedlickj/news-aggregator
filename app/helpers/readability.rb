...
#      puts "best c score:"
#      puts best_candidate[:content_score]
#      puts "best c elem:"
#      if(best_candidate[:elem])
#        puts best_candidate[:elem]
#      else
#        "no element"
#        sleep 1
#      end
      ...
      if(best_candidate[:elem])   #Bugfix for when there is no element
    ...
    def sanitize(node, candidates, options = {})
      ...
      ([node] + node.css("*")).each do |el|
        ...
        if whitelist[el.node_name]
          ...
          begin   #Bugfix - make sure el has text attr
            el.swap(el.text)
          rescue
            puts "Nokogiri failed"
          end
        end
